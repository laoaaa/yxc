document.addEventListener('DOMContentLoaded', () => {
    loadForbiddenWordsWithLabels(); // 调用加载包含标签的禁止词函数
...
function checkForbiddenWords() {
    ...
    let detectedLabels = []; // 创建一个数组用于存储检测到的词及其对应标签

    // 遍历禁止词表，检查输入文本中是否包含禁止词
    forbiddenWordsWithLabels.forEach(({ word, label }) => {
        const regex = new RegExp(`${escapeRegExp(word)}`, 'gi'); // 创建正则表达式用于匹配词语
        if (regex.test(textInput)) {
            detectedWords.add(word); // 将匹配到的禁止词加入集合
            detectedLabels.push({ word, label }); // 将词和标签加入到数组中
    ...
    displayDetectedWords(detectedLabels); // 显示检测到的词及其对应标签
}
...
function displayDetectedWords(detectedLabels) {
    ...
    if (detectedLabels.length > 0) {
        detectedLabels.forEach(({ word, label }) => {
            ...
            listItem.textContent = `${word} - 标签: ${label}`; // 设置列表项内容为“词 - 标签”
